/**
 * AutoGrammar Projects Landing Page & GitHub Cache Engine
 */
...
const CATEGORY_COLORS = {
  'NLP & Compilers': '#a855f7',
  'DSL & Grammar Engines': '#06b6d4',
  'Testing & Fuzzing': '#f43f5e',
  'Execution & Automation': '#10b981',
  'Grammar & Linguistics': '#f59e0b',
  'Portals & Governance': '#6366f1'
};
...
async function loadProjectData() {
  const LOCAL_CACHE_KEY = 'ag_github_daily_cache_v1';
...
function renderProjectsGrid() {
  ...
  appState.filteredProjects.forEach(p => {
    const catColor = CATEGORY_COLORS[p.category] || '#a855f7';
    ...
    style: [
      ...
      },
      {
        ...
        style: {
          ...
          'target-arrow-color': 'rgba(168, 85, 247, 0.8)',
...
function renderDependencyMatrix() {
  ...
  let html = `
    <table style="width:100%; border-collapse:collapse; font-size:0.85rem;">
      <thead>
        <tr style="background: rgba(27,32,56,0.8); text-align:left; border-bottom: 1px solid var(--glass-border);">
  ...
  allProjects.forEach(p => {
    ...
    html += `
      <tr style="border-bottom: 1px solid var(--glass-border); transition: background 0.2s ease;" onmouseover="this.style.background='rgba(27,32,56,0.5)'" onmouseout="this.style.background='transparent'">
...
function openProjectModal(projId) {
  ...
  document.getElementById('modal-project-cat').style.color = CATEGORY_COLORS[p.category] || '#a855f7';
  document.getElementById('modal-github-link').href = p.github_url || `https://github.com/autogrammar/${p.id}`;
  ...
  if (appState.activeModalTab === 'task') {
    body.innerHTML = `
      <div style="display:flex; flex-direction:column; gap:1.5rem;">
        <div style="background: rgba(16,20,38,0.8); border: 1px solid var(--glass-border); padding: 1.5rem; border-radius: var(--border-radius-md);">
        ...
        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:1rem;">
          <div style="background: rgba(16,20,38,0.6); padding: 1.25rem; border-radius: var(--border-radius-md); border:1px solid var(--glass-border);">
    ...
    body.innerHTML = `
      <div style="display:flex; flex-direction:column; gap:1.5rem;">
        <div>
          <h4 style="color:var(--accent-violet); margin-bottom:0.75rem;">Wymagane Zależności (Upstream):</h4>
